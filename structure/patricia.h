/**
 * 核心思想是“纵横交织与资产解耦”：纵向路径压缩：跳过无分支位，查找次数与规则量N 解耦
 * 最坏仅 32/128 次，常态仅个位数。
 * 横向微型链表：同网段下用短链表串联离散端口，在时间轴上做并集，换取零内存爆炸。
 * 零拷贝复用：策略无拓扑指针，范围切分时可多处挂载同一实体指针，免除 Double-Free。
 * 高效举例：若树中仅有一条规则，查找时由于路径压缩，while 循环仅执行 1 次。
 */
#ifndef PATRICIA_H
#define PATRICIA_H
...
/**
 * @brief 核心数据面（Fast Path）严格最长匹配单一策略过滤接口
 * @param table 顶层双栈总控制表指针
 * @param addr 进站报文网络层的原始大端二进制源 IP 地址指针（物理单调对齐查询）
 * @param af 地址族（AF_INET 或 AF_INET6），引导流量自动分流至对应的独立树骨架
 * @param proto 传输层网络协议号（标准度量：IPPROTO_TCP, IPPROTO_UDP, IPPROTO_ICMP 等）
 * @param port 传输层流量的解包目标端口号（大端或主机序需与录入时的 port_lo/hi 保持刚性对齐）
 * @return rule_t* 放行凭证指针。若返回非 NULL，代表当前流量命中了【最精确掩码节点】内部允许放行的规则实体指针；返回 NULL 代表拒绝或未命中，流量一律丢弃（Drop）
 * @note 数据面（快速路径 Fast Path）高阶特判接口。时间复杂度严格锁死在单向线性下钻的 O(32) 或 O(128)。
 * * @par 使用场景（Usage Scenarios）：
 * 1. 零信任网络精细化安全微隔离（Micro-segmentation）：适用于需要对核心敏感资产（如中心数据库子网）执行局部绝对审计的零信任网络拓扑。
 * 2. 大网段默认放行、小网段精准特判/拉黑：当业务系统需要实现“全局默认开放大网段权限（如 /16），但针对特定的敏感核心子网（如 /24）或单一主机（如 /128）执行独立的、不受外层策略干扰的端口权限重构”时。
 * * @par 备注：与并集广域过滤（acl_lookup）的底层核心差别：
 * 1. 过滤机制的代数拓扑差异：
 * -  acl_lookup（纵向广域并集）：采用“纵向分布式或门”机制。流量沿二进制路径下钻时，【只要沿途经过的任何一个有效大网段（如 /16）】端口匹配通过，就会立刻提前短路放行。它代表的是路径上所有相交网段规则的**并集（OR 叠加）**。
 * -  acl_lookup_lpm（严格最长匹配单一特判）：采用“局部精确短路”机制。流量下钻时无视沿途一切宏观网段，直接利用 patricia_lpm 瞬间扎到树的最深处，【有且仅揪出掩码最长、最精准匹配的那个单一网络节点】。程序只在这个最精确节点内部遍历横向单链表核对端口。如果该精确节点的端口未对上，【直接拦截（Drop），坚决不向外层任何大网段妥协】。
 * 2. CPU 流水线效能差异：
 * - acl_lookup 在下钻过程中需要频繁对路过的每一个有效节点执行 prefix_match 和横向单链表深度测试。
 * - acl_lookup_lpm 将“纵向下钻”与“横向测试”完全解耦。在下钻时仅做纯粹的按位分支挺进，直到定调出唯一的 LPM 终点后，才执行仅有一次的横向链表端口盲测，进一步减少了 CPU 缓存（L1/L2 Cache）外的不必要指针跳转，具有更高的硬件流水线执行效能。
 */
...
/**隧道业务资源接口 */
/**全局存l3vpn规则的方式，只有有效资源才存入 */
typedef struct rule_l3vpn
{
	rule_t **rule;
	int size;		// 总的开辟空间
	int vaild_size; // 有效rule
} rule_l3vpn_t;
extern rule_l3vpn_t g_l3vpn_rules;
extern patricia_table_t g_ip_table;
// 回收全局rule
...
// 创建rule- 最后统一由 free_g_l3vpn_rules回收
rule_t *rule_new(uint8_t proto, uint16_t port_lo, uint16_t port_hi);
// 插入查表规则-  最后统一由 free_g_ip_table回收
